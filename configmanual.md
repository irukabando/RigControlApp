...
* **コメントの記述ルール**:
* **行全体のコメント**: 行頭に `#` または `;` を記述します。その行は処理から除外されます。
* **行末のインラインコメント**: コマンドや値と同じ行の後ろにメモを書く場合は、**必ず `#**` を使用してください（例: `TX_GET = TX; # PTT取得`）。`;` はCATコマンドの終端文字として扱われるため、インラインコメントには使用できません。
...
* **`{0:D11}`（11桁ゼロ埋め）**: 周波数 7.074MHz（7,074,000Hz）$\rightarrow$ `00007074000`（送信例: `FA00007074000;`）
* **`{0:D9}`（9桁ゼロ埋め）**: 周波数 14.074MHz（14,074,000Hz）$\rightarrow$ `014074000`（送信例: `FA014074000;`）
* **`{0:D8}`（8桁ゼロ埋め）**: 周波数 14.074MHz（14,074,000Hz）$\rightarrow$ `14074000`（送信例: `FA14074000;`）
